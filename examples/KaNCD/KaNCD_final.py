...
import logging
from CDMs import KaNCD, utils_dataset
import torch
...
def main(dataset_name='', use_subset=False, subset_ratio=0.1):
    ...
    # train函数内部会自动保存最佳模型为 f"{dataset_name}_kancd.snapshot"
    cdm.train(train_set, valid_set, epoch_n=25, device="cpu", lr=0.002)
    # 加载最佳模型
    cdm.load(f"{dataset_name}_kancd.snapshot")
