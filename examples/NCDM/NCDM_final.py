...
import logging
from CDMs import NCDM, utils_dataset
import torch
...
def main(dataset_name='', use_subset=False, subset_ratio=0.1):
    ...
    cdm = NCDM(knowledge_n=knowledge_n, exer_n=item_n, student_n=user_n, data_set_name=dataset_name)
    # 修改train函数参数名以匹配NCDM
    # 注意：NCDM模型会在训练过程中自动保存最佳模型
    # 会保存为 f"{data_set_name}_ncdm.snapshot" 和 f"{data_set_name}_ncdm_{epoch_i}.snapshot"
    best_auc = cdm.train(train_set, valid_set, epoch=50, device="cpu")
    print(f'\nbest auc{best_auc}')
    # 加载最佳模型
    cdm.load(f"{dataset_name}_ncdm_best.snapshot")
    auc, accuracy = cdm.eval(test_set)
    print("auc: %.6f, accuracy: %.6f" % (auc, accuracy))


if __name__ == '__main__':
    dataset_list = ['FrcSub','junyi', 'SLP', 'a0910' ]
    # 数据子集选项
    use_subset = False  # 是否使用数据子集，默认为False
    subset_ratio = 0.1  # 使用原始数据的比例，默认为0.01（1%）

    for dataset_name in dataset_list:
        print('\n\n\data_set_name:', dataset_name)
        try:
            # only to accelerate debug
