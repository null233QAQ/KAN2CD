...
import logging
from CDMs import KaNCDori, utils_dataset
import torch
...
def main(dataset_name='', use_subset=False, subset_ratio=0.1):
    ...
    cdm = KaNCDori(exer_n=item_n, student_n=user_n, knowledge_n=knowledge_n, mf_type='gmf',dataset_name=dataset_name, dim=20)
    cdm.train(train_set, valid_set, epoch_n=25, device="cuda", lr=0.002)
    cdm.save(f"{dataset_name}_KaNCDori.snapshot")

    cdm.load(f"best_KaNCDori_{dataset_name}.snapshot")
