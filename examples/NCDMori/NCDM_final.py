...
import logging
from CDMs import NCDMori, utils_dataset
import torch
...
def main(dataset_name='', use_subset=False, subset_ratio=0.1):
    ...
    cdm = NCDMori(knowledge_n=knowledge_n, exer_n=item_n, student_n=user_n,dataset_name=dataset_name)
    cdm.train(train_set, valid_set, epoch=25, device="cuda")
    cdm.save(f"{dataset_name}_NCDMori.snapshot")
    cdm.load(f"best_NCDMori_{dataset_name}.snapshot")
