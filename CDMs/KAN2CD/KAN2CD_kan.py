# more efficient KAN version -kan
...
class Net(nn.Module):
    ...
    def __init__(self, knowledge_n, exer_n, student_n):
        ...
        self.kan_embed_e1 = qkan.kan.QKAN(e_size)
        self.kan_embed_e2 = qkan.kan.QKAN(e_size)
        self.kan_embed_e3 = qkan.kan.QKAN(e_size)
        self.kan_embed_e4 = qkan.kan.QKAN(e_size)
        self.kan_embed_e5 = qkan.kan.QKAN(e_size)

        k_size = [self.know_num , self.embed_dim,1]
        self.kan_embed_k1 = qkan.kan.QKAN(k_size)
        self.kan_embed_k2 = qkan.kan.QKAN(k_size)
        self.kan_embed_k3 = qkan.kan.QKAN(k_size)
        self.kan_embed_k4 = qkan.kan.QKAN(k_size)
        self.kan_embed_k5 = qkan.kan.QKAN(k_size)

        s_size = [self.stu_num,self.embed_dim,1]
        self.kan_embed_s1 = qkan.kan.QKAN(s_size)
        self.kan_embed_s2 = qkan.kan.QKAN(s_size)
        self.kan_embed_s3 = qkan.kan.QKAN(s_size)
        self.kan_embed_s4 = qkan.kan.QKAN(s_size)
        self.kan_embed_s5 = qkan.kan.QKAN(s_size)

        # self.ori_kan = KAN(width=[self.prednet_input_len, 32, 1], grid=5, k=3, seed=0)
        self.kan_param = [15,1]
        self.fusion_kan = qkan.kan.QKAN(self.kan_param, grid_size=15)
