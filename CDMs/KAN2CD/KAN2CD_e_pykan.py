...
# from ori_kan import *
from ori_kan.ori_KAN import KAN


def t_to_1(tensor, num_classes):
    return torch.nn.functional.one_hot(tensor, num_classes=num_classes).float()

def multi_hot_to_list(multi_hot_tensor):
    return torch.nonzero(multi_hot_tensor).squeeze().tolist()
...
class Net(nn.Module):
    ...
    def __init__(self, knowledge_n, exer_n, student_n):
        ...
        self.embed_e1 = nn.Embedding(self.exer_num, self.embed_dim)
        self.embed_e2 = nn.Embedding(self.exer_num, self.embed_dim)
        self.embed_e3 = nn.Embedding(self.exer_num, self.embed_dim)
        self.embed_e4 = nn.Embedding(self.exer_num, self.embed_dim)
        self.embed_e5 = nn.Embedding(self.exer_num, self.embed_dim)

        self.e1_kan = KAN([self.embed_dim,1])
        self.e2_kan = KAN([self.embed_dim,1])
        self.e3_kan = KAN([self.embed_dim, 1])
        self.e4_kan = KAN([self.embed_dim, 1])
        self.e5_kan = KAN([self.embed_dim, 1])
        ...
        self.embed_s1 = nn.Embedding(self.stu_num, self.embed_dim)
        self.embed_s2 = nn.Embedding(self.stu_num, self.embed_dim)
        self.embed_s3 = nn.Embedding(self.stu_num, self.embed_dim)
        self.embed_s4 = nn.Embedding(self.stu_num, self.embed_dim)
        self.embed_s5 = nn.Embedding(self.stu_num, self.embed_dim)

        self.s1_kan = KAN([self.embed_dim, 1])
        self.s2_kan = KAN([self.embed_dim, 1])
        self.s3_kan = KAN([self.embed_dim, 1])
        self.s4_kan = KAN([self.embed_dim, 1])
        self.s5_kan = KAN([self.embed_dim, 1])
    ...
    def forward(self, stu_id, exe_id, know_id):
        ...
        e_one_hot = exe_id
        e1_embed = self.embed_e1(e_one_hot)
        e2_embed = self.embed_e2(e_one_hot)
        e3_embed = self.embed_e3(e_one_hot)
        e4_embed = self.embed_e4(e_one_hot)
        e5_embed = self.embed_e5(e_one_hot)

        e1_embed = self.e1_kan(e1_embed)
        e2_embed = self.e2_kan(e2_embed)
        e3_embed = self.e3_kan(e3_embed)
        e4_embed = self.e4_kan(e4_embed)
        e5_embed = self.e5_kan(e5_embed)
        ...
        s_one_hot = stu_id
        s1_embed = self.embed_s1(s_one_hot)
        s2_embed = self.embed_s2(s_one_hot)
        s3_embed = self.embed_s3(s_one_hot)
        s4_embed = self.embed_s4(s_one_hot)
        s5_embed = self.embed_s5(s_one_hot)
        s1_embed = self.s1_kan(s1_embed)
        s2_embed = self.s2_kan(s2_embed)
        s3_embed = self.s3_kan(s3_embed)
        s4_embed = self.s4_kan(s4_embed)
        s5_embed = self.s5_kan(s5_embed)
...
class KAN2CD_e_pykan(CDM):
    ...
    def train(self, train_data, test_data=None, epoch=10, device="cpu", lr=0.002, silence=False):
        ...
        for epoch_i in range(epoch):
            ...
            self.save(f"KAN2CD_embed_{self.dataset_name}_{epoch_i}.snapshot")
            ...
            if test_data is not None:
                ...
                if auc > best_auc:
                    ...
                    best_acc = accuracy
                    best_epoch = epoch_i
                    self.save(f"best_KAN2CD_embed_{self.dataset_name}.snapshot")
                print(f'\nbest auc{best_auc}')
